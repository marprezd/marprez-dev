...
export default function Header(): JSX.Element {
  ...
  useEffect(() => {
    let downDelay = (logoRef.current as any)?.offsetTop ?? 0
    let upDelay = 64
    ...
    function updateLogoStyles(): void {
      if (!isHomePage) {
        return
      }
      ...
      setProperty({
        property: '--logo-border-transform',
        value: borderTransform,
      })
      setProperty({
        property: '--logo-border-opacity',
          ...
          <Container>
            <div className="relative flex gap-4">
              ...
              <div className="flex justify-end md:flex-1">
                <Link
                  href="/hire-me"
                  className="pointer-events-auto mr-2 hidden items-center rounded-full bg-white/90 px-4 py-2 text-sm font-medium text-gray-800 shadow-lg shadow-gray-800/5 ring-1 ring-gray-900/5 backdrop-blur dark:bg-gray-800/90 dark:text-gray-200 dark:ring-white/10 dark:hover:ring-white/20 md:block"
                >
                  Hire me
                </Link>
